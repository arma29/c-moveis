...
#     def __init__(self, freq):
#         self.freq = freq
#         self.n0 = LeeAreaType.SubUrban[0]
#         self.p0 = LeeAreaType.SubUrban[1]
#         self.txH = 50.0
#         self.rxH = 1.5
#     def pathloss(self,dist):
...
# class EricssonModel(object): #frequenc limitation :/
#     """Ericcson Model"""
#     def __init__(self, freq):
#         self.freq = freq
...
#         return PL
class Cost231Model(object): #Good candidate
...
class Cost231HataModel(object): #Good candidate
    ...
    def dist_path(self, path):
        if (self.checkFreq):
            if (self.freq<=150 or self.freq>=2000):
                raise ValueError('The frequency range for Cost-Hata Extension Model is 150MHz-2000Mhz')
        f,hm,hb = self.freq,self.rxH,self.txH
        ar=(1.1*np.log10(f)-0.7)*hm-(1.56*np.log10(f)-0.8)
        C = 3 if  (self.areaKind==AreaKind.Urban) else 0
        d = pow(10, ((path - 46.3 - C -33.9*np.log10(f) +13.82*np.log10(hb) + ar)/(44.9-6.55*np.log10(hb))))
        return d
# class OkumuraHataModel(object): #frequency limitation
#     """Okumura-Hata Model"""
#     def __init__(self, freq):
#        self.freq = freq
#        self.rxH =1.5
...
# class Ecc33Model(object): #frequency limitation
#     def __init__(self, freq):
#        self.freq = freq
#        self.rxH =1.5
#        self.txH = 50.0
...
class SuiModel(object): #good candidate
    def __init__(self, freq):
       self.freq = freq
       self.rxH =1.5
       self.txH = 50.0
       self.terrainKind = TerrainKind.C
       self.checkFreq = True
    ...
    def pathloss(self,dist):
        ...
        txH, rxH, f,d = self.txH, self.rxH, self.freq, np.multiply(dist,1000) #meters
    ...
    def dist_path(self, path):
        if (self.checkFreq):
            if (self.freq<=1900 or self.freq>=11000):
                raise ValueError('The frequency range for SUI Model is 1900 MHz-11.000 Mhz')
        txH, rxH, f= self.txH, self.rxH, self.freq #meters
        coef_a = (4.6,0.0075,12.6,-10.8)
        coef_b = (4.0,0.0065,17.1,-10.8)
        coef_c = (3.6,0.005,20,-20)
        s = self.shadowFading
        ...
        Xf = 6 * np.log10(f / 2000.)
        Xh = XhCF * np.log10(rxH / 2)
        dr = pow(10, ((path - Xf - Xh - s - A)/(10*y)))
        d = np.multiply(dr,100) #distance em metros
        d = np.multiply(d, 1./1000) #em km
